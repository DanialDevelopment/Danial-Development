...
const config = {
    description: 'Demote a user on roblox.',
    aliases: [],
    usage: '<username>',
    category: 'Roblox'
}
...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        .setColor('GREEN')
        .setTitle(`Ranked!`)
        .setDescription(`${user} has been demoted to ${rankingInfo.newRole.name} (${rankingInfo.newRole.rank})`)
        ...
        if(process.env.rankinglogs !== 'false'){
            ...
            .setDescription(`${user} (${id}) has been demoted by ${message.author.tag} from ${rankNameInGroup} (${rankInGroup}) to ${rankingInfo.newRole.name} (${rankingInfo.newRole.rank})`)
